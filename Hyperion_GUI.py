# main.py
# ──────────────────────────────────────────────────────────────────────────────
# Bokeh Server App: SSR Emulator (Single & Batch modes)
# - Load trained RF surrogate (joblib)
# - Single prediction (bar)
# - Batch upload CSV/XLSX (time-series + CSV download)
# - Pure Python, no external JS deps
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# 1) Load your trained model (uses your light surrogate class shape/IO)
# ──────────────────────────────────────────────────────────────────────────────
import joblib

class _SSRWrapper:
    """Tiny wrapper to keep interface consistent (predict expects arrays)."""
    def __init__(self, path="ssr_rf_surrogate.pkl"):
...
MODEL_PATH = "ssr_rf_surrogate.pkl"
try:
...
# 2) Styling / Header
# ──────────────────────────────────────────────────────────────────────────────
curdoc().title = "SSR Emulator · RF Surrogate"
...
""", stylesheets=[pulse_shadow_css], width=950 )
...
lat_in   = TextInput(title="Latitude (−90 to 90)", value="40.0", width=220,stylesheets=[textinput_css])
month_sl = Slider(title="Month (1–12)", start=1, end=12, step=1, value=6, width=220,stylesheets=[slider_style])
...
# ──────────────────────────────────────────────────────────────────────────────
file_in = FileInput(accept=".csv,.xlsx,.xls", width=360, stylesheets=[file_input_style],)
...
batch_panel = TabPanel(child=column(
    row(file_in,batch_predict_btn, download_btn),
    row(cloud_in, aod_in,lat_in, month_sl),
    ts,
    table,
    status
